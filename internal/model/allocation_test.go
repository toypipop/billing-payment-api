...
func TestPlanPayment(t *testing.T) {
	...
	before := append([]Invoice(nil), invoices...)
	for _, tc := range []struct {
		...
		{"overpayment", 190001, nil, nil, ErrOverpayment},
	} {
		t.Run(tc.name, func(t *testing.T) {
			plan, err := PlanPayment(invoices, tc.amount)
	...
	if _, err := PlanPayment(invoices[3:], 1); !errors.Is(err, ErrNoOutstandingInvoices) {
		t.Fatalf("paid invoices: %v", err)
	}
	if _, err := PlanPayment(nil, 1); !errors.Is(err, ErrNoOutstandingInvoices) {
		t.Fatalf("empty invoices: %v", err)
	}
	for _, amount := range []int64{0, -1} {
		if _, err := PlanPayment(invoices, amount); err == nil {
...
func TestPlanPaymentLargeBalances(t *testing.T) {
	plan, err := PlanPayment([]Invoice{
		{ID: 1, InvoiceNumber: "A", TotalAmountCents: math.MaxInt64},
		{ID: 2, InvoiceNumber: "B", TotalAmountCents: math.MaxInt64},
	}, math.MaxInt64)
	if err != nil || len(plan) != 1 || plan[0].AmountCents != math.MaxInt64 || plan[0].Invoice.Status() != InvoicePaid {
